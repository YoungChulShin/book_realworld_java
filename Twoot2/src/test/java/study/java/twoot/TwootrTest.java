...
class TwootrTest {
    ...
    void shouldBeAbleToAuthenticateUser() {
        ...
        // 로그온 메서드는 새 엔드포인트 반환
        // 엔드포인트 유효성을 확인하는 어서션
        Optional<SenderEndPoint> senderEndPoint = twootr.onLogon(TestData.USER_ID, receiverEndPoint);
        Assertions.assertTrue(senderEndPoint.isPresent());
    }

    @Test
    void shouldNotAuthenticateUnknownUser() {
        Optional<SenderEndPoint> senderEndPoint = twootr.onLogon(TestData.NOT_A_USER, receiverEndPoint);
